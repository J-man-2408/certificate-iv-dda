...
FROM students
WHERE first_name IN ('amanda', 'ryan')
GROUP BY first_name; 
...
WHERE enrolments.student_id IN (
	SELECT id FROM students 
...
JOIN enrolments ON enrolments.students_id = students.id;
...
SELECT * FROM students ORDER BY date_of_birth LIMIT 1;
SELECT `name` FROM classes WHERE `name` LIKE '%data%'; 